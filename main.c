...
// Struktur menyimpan informasi buku
struct Buku
{
    unsigned int id;
    char judul[100];
...
// Struktur menyimpan informasi pengguna
struct Pengguna
{
    char username[100];
    char password[100];
...
struct Pengguna user[MAX_USER] = {{"admin", "adminklp3", 1}, {"user", "userklp3", 0}};
int jumlah_buku = 0;
int jumlah_user = 2;
...
int main()
{
    bacaDataBuku(); // Membaca data buku dari file "data_perpustakaan.txt"
...
void login()
{
    ...
    if (!loggedIn)
    {
        printf("Maaf! Login anda gagal. Silakan coba lagi.\n");
        login();
    }
}
...
void tambahBuku()
{
    ...
    {
        printf("Maaf, anda tidak dapat menambahkan buku lagi, kapasitas sudah penuh.\n");
        return;
    }
    ...
    jumlah_buku++;
    printf("Hore! Buku berhasil ditambahkan.\n");
...
void pinjamBuku()
{
    ...
    int id_buku;
    printf("Silahkan masukkan ID buku yang ingin dipinjam: ");
    scanf("%d", &id_buku);
    clearBuffer();
    ...
    for (int i = 0; i < jumlah_buku; ++i)
    {
        if (buku[i].id == id_buku)
        {
            ...
            {
                printf("Maaf, buku yang anda maksud sedang tidak tersedia.\n");
                return;
            }
        }
    }
